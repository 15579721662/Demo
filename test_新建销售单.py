...
def pytest_runtest_makereport(item, call):
    ...
    if report.when == "call":
        # 添加日志信息到报告中
        log_content = "\n".join([f"{record.asctime} - {record.levelname} - {record.message}" for record in logging.getLogger().handlers[0].records])
        extra.append(extras.text(log_content, name="日志信息"))
    report.extra = extra
...
def test_select_option(browser):
 try:
    ...
    logger.info("欢迎信息: %s", welcome_text)
